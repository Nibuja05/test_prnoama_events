...
if not PlayerTables then
    print('[PlayerTables] creating PlayerTables')
    PlayerTables = class({})
end
...
function PlayerTables:start()
    ...
    CustomGameEventManager:RegisterListener("PlayerTables_Connected",
...
function PlayerTables:PlayerTables_Connected(args)
    ...
    for k, v in pairs(PlayerTables.subscriptions) do
        if v[pid] then
            if player then
                CustomGameEventManager:Send_ServerToPlayer(player, "pt_fu", {
                    name = k,
                    table = PlayerTables.tables[k]
                })
            end
        end
    end
...
function PlayerTables:CreateTable(tableName, tableContents, pids)
    ...
    for k, v in pairs(pids) do
        ...
        if pid >= 0 and pid < DOTA_MAX_TEAM_PLAYERS then
            ...
            print("[playertables.lua] Warning: Pid value '" .. pid .. "' is not an integer between [0," ..
                      DOTA_MAX_TEAM_PLAYERS .. "].  Ignoring.")
        end
    end
end
...
function PlayerTables:AddPlayerSubscription(tableName, pid)
    ...
    if not oldPids[pid] then
        if pid >= 0 and pid < DOTA_MAX_TEAM_PLAYERS then
            ...
            print("[playertables.lua] Warning: Pid value '" .. v .. "' is not an integer between [0," ..
                      DOTA_MAX_TEAM_PLAYERS .. "].  Ignoring.")
        end
    end
end
...
function PlayerTables:DeleteTableKey(tableName, key)
    ...
    if table[key] ~= nil then
        ...
        for pid, v in pairs(pids) do
            local player = PlayerResource:GetPlayer(pid)
            if player then
                CustomGameEventManager:Send_ServerToPlayer(player, "pt_kd", {
                    name = tableName,
                    keys = {
                        [key] = true
                    }
...
function PlayerTables:SetTableValue(tableName, key, value)
    ...
    if not self:equals(table[key], value) then
        ...
        for pid, v in pairs(pids) do
            local player = PlayerResource:GetPlayer(pid)
            if player then
                CustomGameEventManager:Send_ServerToPlayer(player, "pt_uk", {
                    name = tableName,
                    changes = {
                        [key] = value
                    }
...
function PlayerTables:CreatePlayerTable(tableName, tableContents, pid)
    local playerTableName = tableName .. tostring(pid)

    PlayerTables:CreateTable(playerTableName, tableContents, {pid})
end

function PlayerTables:DeletePlayerTable(tableName, playerID)
    local playerTableName = tableName .. tostring(playerID)

    PlayerTables:DeleteTable(playerTableName)
end

function PlayerTables:PlayerTableExists(tableName, playerID)
    local playerTableName = tableName .. tostring(playerID)

    return self.tables[playerTableName] ~= nil
end

function PlayerTables:SetPlayerValue(tableName, playerID, key, value)
    local playerTableName = tableName .. tostring(playerID)

    PlayerTables:SetTableValue(playerTableName, key, value)
end

function PlayerTables:SetPlayerValues(tableName, playerID, changes)
    local playerTableName = tableName .. tostring(playerID)

    PlayerTables:SetTableValues(playerTableName, changes)
end

function PlayerTables:DeletePlayerKeys(tableName, playerID, keys)
    local playerTableName = tableName .. tostring(playerID)

    PlayerTables:DeleteTableKeys(playerTableName, keys)
end

function PlayerTables:DeletePlayerKey(tableName, playerID, key)
    local playerTableName = tableName .. tostring(playerID)

    PlayerTables:DeleteTableKey(playerTableName, key)
end

function PlayerTables:GetPlayerValue(tableName, playerID, key)
    local playerTableName = tableName .. tostring(playerID)

    return PlayerTables:GetTableValue(playerTableName, key)
end

function PlayerTables:GetAllPlayerValues(tableName, playerID)
    local playerTableName = tableName .. tostring(playerID)

    return PlayerTables:GetTableValues(playerTableName)
end
